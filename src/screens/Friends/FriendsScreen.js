...
// Manual icons as components
const ChevronLeftIcon = ({ size = 28, color = "#fff" }) => (
...
const SearchIcon = ({ size = 22, color = "#999" }) => (
  <View style={{ width: size, height: size, justifyContent: 'center', alignItems: 'center' }}>
    <View style={{
      width: size * 0.7,
      height: size * 0.7,
...
const MenuDotsIcon = ({ size = 20, color = colors.primary }) => (
          ...
          <Menu
            ...
            anchor={
              <TouchableOpacity onPress={() => setMenuVisible(item.id)}>
                <MenuDotsIcon size={20} color={colors.primary} />
              </TouchableOpacity>
            }
  ...
  return (
    <Provider>
      <View style={styles.container}>
        {/* Replace header with your Header component */}
        <Header
          title="Friends"
          leftIcon={<ChevronLeftIcon size={28} color="#fff" />}
          onLeftPress={() => navigation.goBack()}
          gradientColors={["#F9F871", "#F28A47", "#DE5C76"]}
        />
