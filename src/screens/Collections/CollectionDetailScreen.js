...
// ✅ Manual icons
import MenuIcon from '../../../assets/icons/more-dots.png';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            {coll.cover?.url && <Image source={{ uri: coll.cover.url }} style={styles.coverImage} />}
        ...
        renderItem={({ item }) => (
          <View style={styles.postCard}>
            ...
            <Text style={styles.postDate}>
              {new Date(item.createdAt?.seconds ? item.createdAt.seconds * 1000 : Date.now()).toLocaleString()}
            </Text>
          </View>
        )}
      />

      {/* ✅ Dropdown menu using manual icons */}
      {showMenu && (
        <TouchableOpacity
